...
namespace WindowsFormsApp3
{
    public partial class MainForm : Form
    {
        ...
        private void PrintZoomedPatternImage()
        {
            ...
            imgEditor = new Bitmap(imgOut.Width * (editorPixelSize + editorPixelSpacing), imgOut.Height * (editorPixelSize + editorPixelSpacing));
            Graphics graphics = Graphics.FromImage(imgEditor);
            for (int y = 0; y < imgOut.Height; y++)
            {
                for (int x = 0; x < imgOut.Width; x++)
                {
                    graphics.FillRectangle(new SolidBrush(imgOut.GetPixel(x, y)), x * (editorPixelSize + editorPixelSpacing), y * (editorPixelSize + editorPixelSpacing), editorPixelSize, editorPixelSize);
                }
            }
