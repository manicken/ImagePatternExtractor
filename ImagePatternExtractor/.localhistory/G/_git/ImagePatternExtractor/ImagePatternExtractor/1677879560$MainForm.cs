...
namespace WindowsFormsApp3
{
    public partial class MainForm : Form
    {
        ...
        private void PrintZoomedPatternImage()
        {
            ...
            int editorPixelSize = Int32.Parse(txtEditorPixelSize.Text);
            int outPixelSpacing = Int32.Parse(txtEditorPixelSpacing.Text);
            imgEditor = new Bitmap(imgOut.Width * (editorPixelSize + outPixelSpacing), imgOut.Height * (editorPixelSize + outPixelSpacing));
            Graphics graphics = Graphics.FromImage(imgEditor);
            for (int y = 0; y < imgOut.Height; y++)
            {
                for (int x = 0; x < imgOut.Width; x++)
                {
                    graphics.FillRectangle(new SolidBrush(imgOut.GetPixel(x, y)), x * (editorPixelSize + outPixelSpacing), y * (editorPixelSize + outPixelSpacing), editorPixelSize, editorPixelSize);
                }
            }
