...
namespace WeaveImagePatternExtractor
{
    public partial class MainForm : Form
    {
        ...
        {
            textBox1.Text = "Selection: " + rect.ToString();
        }
        ...
        private void PrintPreviewPatternImage()
        {
            ...
            int imgPatternWidth = chkSkipLastColumn.Checked ? imgPattern.Width - 1 : imgPattern.Width;
            for (int xc=0;xc<xCopies;xc++)
            {
                for (int yc=0;yc<yCopies;yc++)
                {
                    for (int y = 0; y < imgPattern.Height; y++)
                    {
                        if (xc == (xCopies - 2)) imgPatternWidth = imgPattern.Width;
                        for (int x = 0; x < imgPatternWidth; x++)
                        {
                            int x2 = x * pixelTotalSize + xc* imgPatternWidth * pixelTotalSize;
