...
namespace WindowsFormsApp3
{
    public partial class MainForm : Form
    {
        ...
        private void btnExtract_Click(object sender, EventArgs e)
        {
            ...
            imgOut = new Bitmap(xParts* outPixelSize, yParts* outPixelSize);

            Graphics graphics = Graphics.FromImage(imgOut);
            Brush brush1 = new SolidBrush(lblLargerThanThresholdColor.BackColor);
            Brush brush2 = new SolidBrush(lblLargerThanThresholdColor.BackColor);
            picOut.Image = null;
            for (int y = 0; y < yParts; y++)
            {
                for (int x = 0; x < xParts; x++)
                {
                    ...
                    if (nr > threshold)
                    {
                        graphics.FillRectangle(brush1, x * outPixelSize, y * 10outPixelSize outPixelSize, outPixelSize);
                    }
                    else
                    {
                        graphics.FillRectangle(brush2, x * outPixelSize, y * outPixelSize, outPixelSize, outPixelSize);
                    }
                }
            }
            picOut.Image = imgOut;
