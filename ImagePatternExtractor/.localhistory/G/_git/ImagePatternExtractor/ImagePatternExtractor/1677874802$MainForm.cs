...
namespace WindowsFormsApp3
{
    public partial class MainForm : Form
    {
        ...
        private void btnExtract_Click(object sender, EventArgs e)
        {
            ...
            //string th_bgc = txtThresholdBiggerThanChar.Text;
            //string th_ec = txtThresholdElseChar.Text;
            ...
            for (int y = 0; y < yParts; y++)
            {
                for (int x = 0; x < xParts; x++)
                {
                    ...
                    if (nr > threshold)
                    {
                        //FillOutImage((int)(x * xMult), (int)(y * yMult), (int)xMult, (int)yMult, lblLargerThanThresholdColor.BackColor);
                        FillOutImage(x * 10, y * 10, 10, 10, lblLargerThanThresholdColor.BackColor);
                        //imgOut.SetPixel(x, y, lblLargerThanThresholdColor.BackColor);
                        
                        dgv.Rows[y].Cells[x].Style.BackColor = lblLargerThanThresholdColor.BackColor;
                        
                        //rtxtOut.AppendText(th_bgc);
                    }
                    else
                    {
                        //FillOutImage((int)(x * xMult), (int)(y * yMult), (int)xMult, (int)yMult, lblThresholdElseChar.BackColor);
                        FillOutImage(x * 10, y * 10, 10, 10, lblThresholdElseChar.BackColor);
                        //imgOut.SetPixel(x, y, lblThresholdElseChar.BackColor);
                        dgv.Rows[y].Cells[x].Style.BackColor = lblThresholdElseChar.BackColor;
                        //rtxtOut.AppendText(th_ec);
                    }
                    //rtxtOut.AppendText(nr.ToString() + " ");
                }
                //rtxtOut.AppendText(Environment.NewLine);
            }
            ((ISupportInitialize)dgv).EndInit();
            picOut.Image = imgOut;
            //picOut.SizeMode = PictureBoxSizeMode.
        }
