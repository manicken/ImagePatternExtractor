...
namespace WeaveImagePatternExtractor
{
    ...
    public static class BitmapExt
    {
        ...
        public static Bitmap RemoveIsolatedPixels2(this Bitmap grayscaleImage, int minPixelCount, int size)
        {
            // Create new bitmap to store cleaned image
            Bitmap cleanedImage = new Bitmap(grayscaleImage.Width, grayscaleImage.Height, grayscaleImage.PixelFormat);
            ...
            for (int x = 0; x < grayscaleImage.Width; x++)
            {
                ...
                {
                    // Check if pixel is foreground
                    if (grayscaleImage.GetPixel(x, y).R == 0)
                    {
                        int pixelCount = 0;
                        ...
                        for (int i = -size; i <= size; i++)
                        {
                            for (int j = -size; j <= size; j++)
                            {
                                ...
                                if (neighborX >= 0 && neighborX < grayscaleImage.Width &&
                                    neighborY >= 0 && neighborY < grayscaleImage.Height &&
                                    grayscaleImage.GetPixel(neighborX, neighborY).R == 0)
                                {
                                    pixelCount++;
                                }
                            }
                        ...
                        if (pixelCount < minPixelCount)
                        {
                            cleanedImage.SetPixel(x, y, Color.FromArgb(255, 255, 255));
                        }
                        else
                        {
                            cleanedImage.SetPixel(x, y, Color.FromArgb(255, 0, 0));
                        }
                    }
                    else
                    {
                        cleanedImage.SetPixel(x, y, grayscaleImage.GetPixel(x, y));
                    }
                }
            }
