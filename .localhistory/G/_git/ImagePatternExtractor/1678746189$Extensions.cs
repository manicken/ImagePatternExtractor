...
namespace WeaveImagePatternExtractor
{
    ...
    public static class BitmapExt
    {
        ...
        {
            Bitmap bmGrayscale = new Bitmap(bitmap.Width, bitmap.Height, PixelFormat.Format8bppIndexed);
            Color[] grayScalePalette = new Color[256];
            for (int i = 0; i < 256; i++)
                grayScalePalette[i] = Color.FromArgb(i, i, i);
            bmGrayscale.Palette = new ColorPalette();
            MessageBox.Show(bmGrayscale.Palette.Entries.Length.ToString());
            ...
            try
            {
                ...
                {
                    int average = ((int)bmdSrcRaw[si] + (int)bmdSrcRaw[si + 1] + (int)bmdSrcRaw[si + 2]) / 3;
